...
plot_ORA <- function(x,
                     n_terms = 5L, # number of top terms per module
                     mods = 1:5, # module numbers to plot
                     subset = "GO:MF",
                     rel_heights = c(0.8, 0.2))
{
  # Check input
  if (!(is.numeric(n_terms) | is.infinite(n_terms))) {
    stop("n_terms must be numeric or Inf.")
  }

  if (!all(mods %in% seq_len(nlevels(x$module)))) {
    stop(paste("mods must be a numeric vector with values between 1",
               "and the number of unique modules in x$module."))
  }
  ...
  setDT(x)
  x <- subset(x,
              subset = (gs_subcat %in% subset) & (padj < 0.05) &
